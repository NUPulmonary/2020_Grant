...
```{r}
rna_metadata = read_sheet([redacted],
                          trim_ws = T,
                          .name_repair = "universal", 
...
bds_edits = read.csv("~/Box/RGrant/SCRIPT/manuscript_1/201130_missing_metadata_BDS_edits.csv") %>% 
  dplyr::select(-X)
...
#add superinfection
complete_bal_data = complete_bal_data %>% 
  mutate(super_type = case_when(pna_type %in% c("Other Pneumonia", "Non-Pneumonia Control") ~ NA_character_,
...
#ensure no patients switch diagnosis   
flip_floppers = c()
for(patient in script_bal_data$study_id)
{
  sub = subset(script_bal_data, study_id == patient)
  if(!all(sub$pna_type == sub$pna_type[1]))
  {
    flip_floppers = append(flip_floppers, patient)
  }
}
flip_floppers = unique(flip_floppers)
if(length(flip_floppers) > 0)
{
  stop(paste(length(flip_floppers), "patients change diagnosis"))
}

if(any(is.na(script_bal_data$day_of_intubation) & script_bal_data$pna_type != "Healthy Control"))
{
  
  stop(paste("Missing intubation days for", sum(is.na(script_bal_data$day_of_intubation)), "samples"))
}
if(any(is.na(script_bal_data$age)))
{
  
  stop(paste("Missing ages for", sum(is.na(script_bal_data$age)), "samples"))
}
```
...
tools::texi2pdf("200728_table_2.tex", clean = TRUE)
write.csv(all_pathogens_0, "~/Box/RGrant/SCRIPT/201130_pathogens_list.csv")
```
...
annotation_data = all_days %>% 
  select(Sample, Diagnosis, finite_day_of_intubation, Superinfection) %>% 
  remove_rownames() %>% 
...
all_days_serial = flow_data %>% 
  dplyr::filter(ID %in% serial_patients) %>% 
  select(ID, percent_CD4_total, percent_CD8_total, percent_macs_CD206_high, percent_macs_CD206_low, 
...
all_comps = all_comps %>% 
  ...
  dplyr::mutate(y = c(52, 62, 45, 55, 65, 
                      120, 95, 65, 80, 95)) #had to just do this manually
...
# CairoPDF("~/Box/RGrant/SCRIPT/manuscript_1/201129_flow_plot_D0.pdf",
#     width = max_width_nature * 3,
...
# CairoPDF("~/Box/RGrant/SCRIPT/manuscript_1/201129_flow_plot_nolegend_D0.pdf",
#     width = max_width_nature * 3,
#     height = max_height_nature, 
#     family = "Arial")
...
# CairoPDF("~/Box/RGrant/SCRIPT/manuscript_1/201129_figure_S1.pdf",
...
# CairoPDF("~/Box/RGrant/SCRIPT/manuscript_1/201129_fig4_rough_local.pdf",
#     width = max_width_nature * 3,
#     height = max_height_nature * 1.5, 
#     family = "Arial")
# flow_plot_fig5
# dev.off()
```
...
# CairoPDF("~/Box/RGrant/SCRIPT/manuscript_1/201129_figS3_top_local.pdf",
