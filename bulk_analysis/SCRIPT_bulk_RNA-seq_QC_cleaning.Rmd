...
## Flow data   
```{r}
flow_data = readRDS("/projects/b1038/Pulmonary/rgrant/script_bulk/Analysis/2020-10-23_SCRIPT_flow_plus_clinical_metadata_processed.rds") %>% 
...
clinical_md = readRDS("/projects/b1038/Pulmonary/rgrant/script_bulk/Analysis/2020-10-23_SCRIPT_clinical_metadata_processed.rds")
md = left_join(md, 
               clinical_md, 
               by = c("tc_pt_study_id"), 
               na_matches = "never")
...
## Remove bad samples   
While there is no issue of cross contamination, samples in wells E2-G3 are a duplication of A1-C2. For safety, remove all samples.   
```{r}
...
```   
PC1 is driven positively by lipid metabolism genes, such as FABP4, many of which are AM markers. Negative loadings may suggest MoAM presence: SPP1 is the first hit. More interesting though: it’s driven most strongly in that direction by Il8 and Il1b https://iai.asm.org/content/80/3/1140.short. PC2 is honestly not that different, but seems to have some proliferation character in the negative direction, homeotstatic MP function in the positive (MERTK, STAB1). Also some inflammatory character: F13A1, CCL2.   
   
# Export for high-level analysis   
```{r}
saveRDS(script_bulk, "/projects/b1038/Pulmonary/rgrant/script_bulk/Analysis/201023_script_bulk_des.rds")
saveRDS(script_bulk_samples, "/projects/b1038/Pulmonary/rgrant/script_bulk/Analysis/201023_script_bulk_des_no_hURNA.rds")
```
      
# Conclusions   
1. Although there is some variability in sequencing quality, mapped and assigned reads lead to a very clean dataset with good between-batch correspondence.   
